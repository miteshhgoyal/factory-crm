...
const PurchaseAccountReport = () => {
  ...
  const [filters, setFilters] = useState({
    type: "IN", // Only purchase transactions (Stock IN)
    productName: "",
    clientName: "",
    startDate: "",
    endDate: "",
  ...
  const fetchAllData = useCallback(async () => {
    try {
      ...
      const promises = [
        // Purchase Transactions (Stock IN only)
      ...
      setError("Failed to fetch purchase report data. Please try again.");
      setPurchaseTransactions([]);
      setStockBalance([]);
      setClients([]);
    } finally {
  ...
  const calculations = useMemo(() => {
    ...
    const todayPurchases = purchaseTransactions.filter((t) =>
      isInDateRange(t.date, todayStart)
    );
    const yesterdayPurchases = purchaseTransactions.filter((t) =>
      isInDateRange(t.date, yesterdayStart, todayStart)
    );
    const weekPurchases = purchaseTransactions.filter((t) =>
    ...
    const todayPurchaseQuantity = todayPurchases.reduce(
      (sum, t) => sum + (t.quantity || 0),
      0
    );

    const yesterdayPurchaseValue = yesterdayPurchases.reduce(
      (sum, t) => sum + (t.amount || 0),
      0
    );

    const weekPurchaseValue = weekPurchases.reduce(
      (sum, t) => sum + (t.amount || 0),
      0
    );
    const weekPurchaseQuantity = weekPurchases.reduce(
      (sum, t) => sum + (t.quantity || 0),
      0
    );

    const monthPurchaseValue = monthPurchases.reduce(
      (sum, t) => sum + (t.amount || 0),
      0
    );
    const monthPurchaseQuantity = monthPurchases.reduce(
      (sum, t) => sum + (t.quantity || 0),
      0
    );
    ...
    const purchaseGrowth =
      yesterdayPurchaseValue > 0
        ? ((todayPurchaseValue - yesterdayPurchaseValue) /
            yesterdayPurchaseValue) *
          100
        : 0;

    // Averages
    const avgDailyPurchase = monthPurchaseValue / now.getDate();
    const avgTransactionValue =
      purchaseTransactions.length > 0
        ? totalPurchaseValue / purchaseTransactions.length
        : 0;
    ...
    return {
      ...
      todayTransactionCount: todayPurchases.length,

      // Weekly data
      weekPurchaseValue,
      weekPurchaseQuantity,
      weekTransactionCount: weekPurchases.length,

      // Monthly data
      monthPurchaseValue,
      monthPurchaseQuantity,
      monthTransactionCount: monthPurchases.length,

      // Total data
      totalPurchaseValue,
      totalPurchaseQuantity,
      totalTransactionCount: purchaseTransactions.length,
  ...
  }, [purchaseTransactions, searchTerm, clientFilter]);
  ...
  const exportData = useCallback(() => {
    try {
      ...
      XLSX.writeFile(
        wb,
        `Purchase_Report_${new Date().toISOString().split("T")}.xlsx`
      );
    } catch (error) {
    ...
    return (
      <div className="space-y-6">
        <HeaderComponent
          ...
          subheader="Comprehensive purchase analysis and supplier overview"
  ...
  return (
    <>
      <div className="space-y-6">
        ...
        <div>
          <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
            ...
            Today's Purchase Overview
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <StatCard
              title="Today's Purchase Value"
              value={`₹${
                calculations.todayPurchaseValue?.toLocaleString() || 0
              }`}
              icon={ShoppingCart}
              color="green"
              subtitle={
                calculations.purchaseGrowth !== 0
                  ? `${
                      calculations.purchaseGrowth > 0 ? "+" : ""
                    }${calculations.purchaseGrowth.toFixed(1)}% vs yesterday`
            ...
            <StatCard
              ...
              value={`${
                calculations.todayPurchaseQuantity?.toFixed(0) || 0
              } kg`}
              icon={Weight}
              color="blue"
            ...
            <StatCard
              ...
              icon={Receipt}
              color="purple"
              subtitle="Purchase orders processed"
            />
            <StatCard
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <StatCard
              ...
              value={`₹${
                calculations.weekPurchaseValue?.toLocaleString() || 0
              }`}
              icon={Target}
              color="green"
            ...
            <StatCard
              ...
              value={`₹${
                calculations.monthPurchaseValue?.toLocaleString() || 0
              }`}
              icon={PieChart}
              color="blue"
            ...
            <StatCard
              ...
              value={`₹${
                calculations.totalPurchaseValue?.toLocaleString() || 0
              }`}
              icon={IndianRupee}
              color="purple"
            ...
            <StatCard
              ...
              icon={Activity}
              color="orange"
              subtitle="Average purchase amount"
            />
          </div>
        </div>
            ...
            <div className="space-y-4">
              <div className="bg-green-50 p-4 rounded-lg">
                ...
                <DataRow
                  ...
                  value={`${
                    calculations.monthPurchaseQuantity?.toFixed(0) || 0
                  } kg`}
                  valueColor="text-green-600"
                />
                <DataRow
                ...
                <div className="border-t border-green-200 mt-2 pt-2">
                  <DataRow
                    label="Avg Daily Purchase"
                    value={`₹${
                      Math.round(
                        calculations.avgDailyPurchase
                      )?.toLocaleString() || 0
                    }`}
                    valueColor="text-green-600"
                    bold={true}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    All purchase orders and stock receipts
                  </p>
                </div>
              </div>
                    ...
                    <tr>
                      <td colSpan="8" className="text-center py-12">
                        ...
                        <p className="text-gray-400 text-sm">
                          Try adjusting your filters or add new purchases
                        </p>
                      </td>
                    </tr>
                  )}
      ...
      <Modal
        ...
        subtitle={`Update purchase details (Original unit: ${editFormData.originalUnit})`}
        headerIcon={<Edit />}
        headerColor="green"
        size="md"
      >
